...
const LoginForm = ({inputs, buttonLabel, handleChange, handleSubmit, errors}) => {
                ...
                <form onSubmit={handleSubmit} className={loginStyle.authFormBlock}>
                    <div className={loginStyle.inputContainer}>
                        ...
                        <div className={loginStyle.inputBlock}>
                            {inputs.map((input) => {
                                const hasError = errors?.[input.name]?.length > 0;
                                ...
                                return (
                                    <div key={input.name}>
                                        <InputField
                                            ...
                                            className={`${inputStyle.input} ${hasError ? inputStyle.inputError : ""}`}
                                        />

                                        {errors?.[input.name]?.map((msg, idx) => (
                                            <Text
                                                key={idx}
                                                as="div"
                                                className={loginStyle.errorMessage}
                                                text={msg}
                                            />
                                        ))}
                                    </div>
